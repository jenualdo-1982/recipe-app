...
FRONTEND_DIR = os.path.join(os.path.dirname(BASE_DIR), "front/dist")
...
# --- ИСПРАВЛЕННЫЙ ЭНДПОИНТ ОБНОВЛЕНИЯ ---
...
async def update_recipe(
    ...
    recipe = db.query(models.Recipe).filter(models.Recipe.id == recipe_id).first()
    if not recipe: raise HTTPException(status_code=404, detail="Рецепт не найден")

    recipe.title = title
    recipe.instructions = instructions
    recipe.servings_default = servings_default

    if image and image.filename:
        recipe_folder = os.path.join(MEDIA_DIR, f"recipe_{recipe.id}")
        os.makedirs(recipe_folder, exist_ok=True)
        file_ext = image.filename.split(".")[-1]
        file_name = f"{uuid4()}.{file_ext}"
        file_location = os.path.join(recipe_folder, file_name)
        with open(file_location, "wb") as buffer:
            shutil.copyfileobj(image.file, buffer)
        recipe.image = f"recipe_{recipe.id}/{file_name}"
    ...
    for ing in ingredients_list:
        ...
        if not db_ing:
            ...
            db.add(db_ing)
            db.flush()
        db.add(models.RecipeIngredient(recipe_id=recipe.id, ingredient_id=db_ing.id, amount=ing["amount"], unit=ing["unit"]))
